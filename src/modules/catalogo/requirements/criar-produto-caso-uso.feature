...
Scenario: Produto inválido - Descriçaõ do produto não atende ao tamanho mínimo (10) (Execeção)
    Dado [Um produto com descriçaõ que não atende ao tamanho mínimo]
    Quando [Solicitar a criação de um produto]
    Então [Um erro informando que a descrição do produto não possui um tamanho mínimo válido deve ser apresentado]

Scenario: Produto inválido - Descriçaõ do produto não atende ao tamanho máximo (200) (Execeção)
    Dado [Um produto com descriçaõ que não atende ao tamanho máximo]
    Quando [Solicitar a criação de um produto]
...
Scenario: Produto inválido - Produto não tem um número mínimo válido de categorias (1) (Execeção)
    ...
    Quando [Solicitar a criação de um produto]
    Então [Um erro informando que o produto não tem um número mínimo válido de categorias deve ser apresentado]
...
Scenario: Produto inválido - Produto não tem um número máximo válido de categorias (3) (Execeção)
    ...
    Quando [Solicitar a criação de um produto]
    Então [Um erro informando que o produto não tem um número máximo válido de categorias deve ser apresentado]